...
export default {
  ...
  computed: {
    chartOptions() {
      ...
      const plugins = {
        datalabels: {
          display: false // Esconde os números sobre os pontos
        }
      }
      ...
      if (this.activeTab.tooltip) {
        if (this.activeTab.tooltip?.callbacks?.label) {
          this.activeTab.tooltip.callbacks.label = eval("(" + this.activeTab.tooltip.callbacks.label + ")");
        }

        plugins.tooltip = this.activeTab.tooltip;
      }

      if (this.activeTab.scales?.y?.ticks?.callback) {
        this.activeTab.scales.y.ticks.callback = eval("(" + this.activeTab.scales.y.ticks.callback + ")");
      }
